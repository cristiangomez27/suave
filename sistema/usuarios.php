    ...
    <style>
        ...
        *{ box-sizing:border-box; }
        ...
        body::after{
            ...
            inset:0;
            background:
                radial-gradient(circle at 15% 15%,rgba(255,255,255,.04),transparent 34%),
                radial-gradient(circle at 70% 80%,rgba(255,255,255,.03),transparent 35%);
            pointer-events:none;
...
<body>
    ...
    document.addEventListener("DOMContentLoaded", function () {
        ...
        function setSocketStatus(conectado) {
            ...
            const labelUrl = wsCandidates[wsIndex] || '';
            socketText.textContent = conectado ? `WS conectado (${labelUrl})` : 'WS desconectado';
        }

        function iniciarWebSocket() {
            try {
                const urlActual = wsCandidates[wsIndex] || wsCandidates[0];
                socket = new WebSocket(urlActual);

                socket.onopen = function() {
                    setSocketStatus(true);
                    console.log('WebSocket usuarios conectado:', urlActual);
                };
